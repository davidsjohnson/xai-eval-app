...
function csv_json_get_additional_attributes(page_nr) {
    ...
    let diag = input.SUGGESTED_DIAG[index];
    let cf_label;
    if (diag === "OCDegen") {
        cf_label = "Healthy";
    ...
    const cf_class = cf_label === "OCDegen" ? "unhealthy" : "healthy";
    const cf_label_html = `<span class="${cf_class}">${cf_label}</span>`;

    concept_card_1_title = "<p>Counterfactual 1: " + cf_label_html + "</p>";
    concept_card_1_image = "img/" + input.Example1[index];
    concept_card_1_caption = input.Example1_Caption[index];

    concept_card_2_title = "<p>Counterfactual 2: " + cf_label_html + "</p>";
    concept_card_2_image = "img/" + input.Example2[index];
    concept_card_2_caption = input.Example2_Caption[index];

    concept_card_3_title = "<p>Counterfactual 3: " + cf_label_html + "</p>";

...
function csv_json_get_additional_attributes(page_nr)
{
    ...
    concept_card_1_bullets  = [input.Concept1_Description[index], input.Concept2_Description[index],
                               input.Concept3_Description[index], input.Concept4_Description[index], 
                               input.Concept5_Description[index]];
    attributes = [concept_card_1_title, concept_card_1_bullets];
    return attributes;
}
...
function set_additional_attributes_in_html_page(page_nr, attr)
{
    ...
    let bullets_ul = document.getElementById("concept-card-1-bullets");
    bullets_ul.innerHTML = ""; // Clear existing bullets

    attr[1].forEach((bullet) => {
        if (bullet && bullet.trim() !== "") { // Check for non-empty bullet
            let className = 'healthy-li';
            if (bullet.toLowerCase().includes('unhealthy') || bullet.toLowerCase().includes('ocdegen')) {
                className = 'unhealthy-li';
            }
            let li = document.createElement("li");
            li.className = className;
            li.innerHTML = bullet;
            bullets_ul.appendChild(li);
        }
    });
}
